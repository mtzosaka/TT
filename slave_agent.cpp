...
void SlaveAgent::transfer_file(const std::string& filepath) {
    try {
        ...
        // Generate filename (basename only)
        fs::path path(filepath);
        std::string filename = path.filename().string();
        
        // Calculate checksum (simplified for now)
        std::string checksum = "checksum_placeholder";
        ...
        while (file && chunks_sent < num_chunks) {
            ...
            // Send chunk
            zmq::message_t chunk_msg(bytes_read);
            memcpy(chunk_msg.data(), buffer.data(), bytes_read);
        ...
        std::cout << "File transfer completed: " << filepath << std::endl;
...
std::string SlaveAgent::send_tc_command(const std::string& cmd) {
    ...
    if (!result.has_value()) {
        throw std::runtime_error("No reply for TC command: " + cmd);
    }
