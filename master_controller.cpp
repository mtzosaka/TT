...
bool MasterController::initialize() {
    try {
        ...
        trigger_socket_.bind("tcp://*:" + std::to_string(config_.trigger_port));
        
        // Status receiver
        status_socket_ = zmq::socket_t(context_, zmq::socket_type::pull);
        status_socket_.bind("tcp://*:" + std::to_string(config_.status_port));
        
        // File receiver
        file_socket_ = zmq::socket_t(context_, zmq::socket_type::pull);
        file_socket_.bind("tcp://*:" + std::to_string(config_.file_port));
        
        // Command socket (for sending commands to slave)
        command_socket_ = zmq::socket_t(context_, zmq::socket_type::req);
        command_socket_.connect("tcp://" + config_.slave_address + ":" + 
                               std::to_string(config_.command_port));
