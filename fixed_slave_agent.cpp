...
bool SlaveAgent::initialize() {
    try {
        ...
        std::cout << "DEBUG: Connecting sync socket to " << sync_endpoint << std::endl;
            ...
            if (result.has_value()) {
                ...
                if (cmd["type"] == "command") {
                    ...
                    if (command == "status") {
                        ...
                        try {
                            std::cout << "DEBUG: Preparing for trigger reception..." << std::endl;
